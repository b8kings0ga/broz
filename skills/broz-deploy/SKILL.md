...
Use `scripts/broz-deploy.sh`; do not reimplement the API flow or request a token from the user.

For deployment:

1. Read `references/runtime-contracts.md` for the selected runtime.
2. Run `scripts/broz-deploy.sh deploy PATH` with `--domain NAME` when the user requests a hostname. Runtime auto-detection is preferred.
...
For `status` and `stop`, run the corresponding command against the project path. `stop` is reversible.

Deletion is destructive. Obtain explicit confirmation immediately before running `scripts/broz-deploy.sh delete PATH --yes`. Never add `--yes` without that confirmation. Successful deletion removes the project's `.broz.json` and its local guest credential.
